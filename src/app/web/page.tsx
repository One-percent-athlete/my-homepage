    ...
    <div className="web-world">
      ...
      <section className="web-lab-hero">
        ...
        <div className="build-console" id="process">
          ...
          <div className="build-mode-tabs">{modes.map((item,index)=><button key={item.id} onClick={()=>setActiveMode(index)} className={index===activeMode?"active":""}>{item.label}</button>)}</div>
          ...
          <button className="console-next" onClick={()=>setActiveMode((activeMode+1)%modes.length)}><Play size={14} fill="currentColor"/> Run next module</button>
        </div>
      </section>
