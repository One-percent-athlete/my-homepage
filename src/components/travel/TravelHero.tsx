  ...
  return (
    <section className="travel-log-hero">
      ...
      <div className="route-selector" id="travel-routes">
        <div className="route-tabs">{routes.map((item,index)=><button key={item.name} className={activeRoute===index?"active":""} onClick={()=>setActiveRoute(index)}><span>0{index+1}</span>{item.name}</button>)}</div>
